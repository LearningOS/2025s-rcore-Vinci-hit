...
impl ProcessControlBlockInner {
    ...
    pub fn check_dead_lock(&self)->bool{
        ...
        let mut finish:Vec<(usize,bool)> = self.need.iter().filter_map(|(usize,_)| Some((*usize,false))).collect();
        let len =finish.len() - 1;
        loop{
            for (idx, (i,finish_i)) in finish.iter_mut().enumerate(){
                if *finish_i == false{
                    if let Some((j,_)) = work.iter().enumerate().find(|(j,count)| {
                        let count2 = self.find_need(*i, *j);
                        count2 > 0 && count2 <= **count
                    }){
                        work[j] += self.find_alloc(*i, j);
                        *finish_i = true;
                    }else{
                        if idx == len{
                            return true;
